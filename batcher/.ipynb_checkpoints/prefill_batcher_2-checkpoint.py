...
def collect_batch(user_input_queue, max_batch, max_wait_ms, timeout):
    curr = []
    target_bin = None
...
def prefill_batches_stage(user_input_queue, page_table ,batch_size, runtime):
    ...
    curr_request, bin_size = collect_batch(user_input_queue, max_batch=batch_size,  max_wait_ms=2000, timeout=0.02)
    ...
    page_table.init_request(req_id = req_id, batch_size = batch_size , num_layers = num_layers , seq_len = bin_size, shape = curr_shape, dtype = torch.float16)
