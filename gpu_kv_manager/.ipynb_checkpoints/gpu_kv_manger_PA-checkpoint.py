...
class GPUKVBlockManager:
    ...
    def alloc_block(self, num_elements: int):
        ...
        self.gpu_block_pool[block_id] = torch.empty(
            num_elements,
            dtype=torch.float16,
            device=self.device,
        )

        self.elements_in_gpu_block[block_id] = num_elements
        return block_id
    ...
    def copy_block_from_cpu(self, cpu_tensor: torch.Tensor):
        ...
        self.gpu_block_pool[block_id].copy_(flat, non_blocking=True)
        return block_id
    ...
    def get_block(self, block_id: int) -> torch.Tensor:
        ...
        """
        return self.gpu_block_pool[block_id]
    ...
    def read_blocks_as_tensor(self, block_ids, shape):
        ...
        for bid in block_ids:
            src = self.gpu_block_pool[bid]
            n = self.elements_in_gpu_block[bid]
