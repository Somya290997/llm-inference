...
prompt1 = ''' Can you summarise this paragrapgh, Modern large-scale AI systems have rapidly evolved into complex, distributed computational pipelines that require careful engineering to achieve both speed and accuracy. Over the past decade, the field has shifted from single-machine training to massively parallelized training strategies that involve sophisticated orchestration across GPUs, TPUs, and heterogeneous accelerators. These systems not only need to support multi-billion-parameter models, but must also optimize communication overhead between devices, manage sharded tensors efficiently, and continuously track memory fragmentation, IO bottlenecks, and latency spikes. As models have grown larger, inference itself has become a significant engineering challenge.'''

# def test_block_manager():
#     """Test if block manager preserves data correctly"""
#     from cpu_kv_manager.cpu_kv_blockmanager import CPUKVBlockManager
#     import torch
    
#     mgr = CPUKVBlockManager()
    
#     # Create test tensors
#     k_test = torch.randn(1, 8, 12, 128, dtype=torch.float16)
#     v_test = torch.randn(1, 8, 12, 128, dtype=torch.float16)
    
#     print("[Test] Writing KV to CPU blocks...")
#     k_ids, v_ids = mgr.write_layer(k_test, v_test)
    
#     print("[Test] Reading KV from CPU blocks...")
#     k_read, v_read = mgr.read_layer(k_ids, v_ids, k_test.shape, 'cpu')
    
#     # Compare
#     k_match = torch.allclose(k_test, k_read, atol=1e-5)
#     v_match = torch.allclose(v_test, v_read, atol=1e-5)
    
#     print(f"[Test] K matches: {k_match}")
#     print(f"[Test] V matches: {v_match}")
#     print(f"[Test] K max diff: {(k_test - k_read).abs().max().item()}")
#     print(f"[Test] V max diff: {(v_test - v_read).abs().max().item()}")
    
#     assert k_match and v_match, "Block manager test FAILED!"
#     print("[Test] ✅ Block manager test PASSED!")

# # Run before main code
# test_block_manager()


runtime.submit_request(req_id=req_id1,prompt=prompt1)
runtime.submit_request(req_id=2,prompt=prompt2)
