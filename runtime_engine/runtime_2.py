...
class Runtime:
    ...
    def __init__(self):
        ...
        self.prefill_batcher = queue.Queue()
        self.decode_batcher = queue.Queue()
        ...
        threading.Thread(target=self.prefill_batcher_worker, args=() , daemon=True).start()
        threading.Thread(target=self.decode_batcher_worker, args=() , daemon=True).start()
    ...
    # prefill batches
    def prefill_batcher_worker(self):
        ...
        while True:
            ...
            while len(requests) < prefill_batch_size or (time.time()-start_time) * 1000 < time_to_wait:
                try:
                    req = self.user_input_queue.get()
                    requests.append(req)
                except queue.Empty:
                    pass
    ...
    def prefill_worker(self):
        while True:
            ...
            prefill_stage(req_id=req_id,prompt=prompt,page_table=self.page_table,cpu_kv_manager=self.cpu_kv_manager, kv_write_to_cpu_queue=self.kv_write_to_cpu_queue)
